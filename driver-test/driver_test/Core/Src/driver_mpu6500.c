/*
 * driver_mpu6500.c
 *
 *  Created on: 2026. 4. 12.
 *      Author: yhj02
 */

#include "driver_mpu6500.h"
#include <math.h>

#define MPU6500_ADDR (0x68 << 1)
#define MPU6500_WHOAMI 0x75
#define MPU6500_DATA 0x3B

#define COMP_ALPHA 0.98f
#define UPDATE_SEC 0.001f


static mpu6500_t accel;
static mpu6500_t gyro;
static mpu6500_conv_t acc_cnv;
static mpu6500_conv_t gyr_cnv;
static mpu6500_conv_t gyr_offset;
static mpu6500_euler_t euler;

static uint8_t mpu6500_buf[14];

const mpu6500_euler_t *mpu6500_get_euler(void){	return &euler; }

void mpu6500_write(uint16_t memaddr, uint8_t * write_data, uint16_t size)
{
	HAL_I2C_Mem_Write(&hi2c1, MPU6500_ADDR, memaddr, 1, write_data, size, 100);
}

void mpu6500_read(uint16_t memaddr, uint8_t * read_data, uint16_t size)
{
	HAL_I2C_Mem_Read(&hi2c1, MPU6500_ADDR, memaddr, 1, read_data, size, 100);
}

void mpu6500_read_IT(uint16_t memaddr, uint8_t * read_data, uint16_t size)
{
	HAL_I2C_Mem_Read_IT(&hi2c1, MPU6500_ADDR, memaddr, 1, read_data, size);
}

void mpu6500_parse(void)
{
	// 가속도 XYZ (0x3B ~ 0x40)
	accel.x = (int16_t)(mpu6500_buf[0] << 8 | mpu6500_buf[1]);
	accel.y = (int16_t)(mpu6500_buf[2] << 8 | mpu6500_buf[3]);
	accel.z = (int16_t)(mpu6500_buf[4] << 8 | mpu6500_buf[5]);

	// buf[6], buf[7]은 온도 → 스킵

	// 자이로 XYZ (0x43 ~ 0x48)
	gyro.x = (int16_t)(mpu6500_buf[8]  << 8 | mpu6500_buf[9]);
	gyro.y = (int16_t)(mpu6500_buf[10] << 8 | mpu6500_buf[11]);
	gyro.z = (int16_t)(mpu6500_buf[12] << 8 | mpu6500_buf[13]);
}

// 가속도(6) + 온도(2, 스킵용) + 자이로(6) = 14바이트 연속 읽기
void mpu6500_read_data(void)
{
    mpu6500_read(MPU6500_DATA, mpu6500_buf, 14);
    mpu6500_parse();
}

void mpu6500_read_data_IT(void)
{
    mpu6500_read_IT(MPU6500_DATA, mpu6500_buf, 14);
}


// ±16g  → 2048 LSB/g   → raw / 2048.0 = g단위
// ±2000dps → 16.4 LSB/(°/s) → raw / 16.4 = °/s단위
void mpu6500_conv_data(void)
{
    acc_cnv.x = accel.x / 2048.0f;
    acc_cnv.y = accel.y / 2048.0f;
    acc_cnv.z = accel.z / 2048.0f;

    gyr_cnv.x = gyro.x / 16.4f - gyr_offset.x;
    gyr_cnv.y = gyro.y / 16.4f - gyr_offset.y;
    gyr_cnv.z = gyro.z / 16.4f - gyr_offset.z;
}

void mpu6500_calibrate(int samples)
{
	gyr_offset.x = 0;
	gyr_offset.y = 0;
	gyr_offset.z = 0;

    for(int i = 0; i < samples; i++)
    {
        mpu6500_read_data();

        gyr_offset.x += gyro.x / 16.4f;
		gyr_offset.y += gyro.y / 16.4f;
		gyr_offset.z += gyro.z / 16.4f;

        HAL_Delay(2);
    }

    gyr_offset.x /= samples;
    gyr_offset.y /= samples;
    gyr_offset.z /= samples;
}

void mpu6500_complementary_filter(float dt)
{
    // 가속도계로 각도 계산 (라디안 → 도)
    float accel_roll  = atan2f(acc_cnv.y, acc_cnv.z) * 57.2958f;
    float accel_pitch = atan2f(-acc_cnv.x, sqrtf(acc_cnv.y * acc_cnv.y + acc_cnv.z * acc_cnv.z)) * 57.2958f;

    // 상보필터 적용
    euler.roll  = COMP_ALPHA * (euler.roll  + gyr_cnv.x * dt) + (1.0f - COMP_ALPHA) * accel_roll;
    euler.pitch = COMP_ALPHA * (euler.pitch + gyr_cnv.y * dt) + (1.0f - COMP_ALPHA) * accel_pitch;

    // yaw는 자이로 적분만 (지자기 센서 없음)
    euler.yaw += gyr_cnv.z * dt;
}

//if you read data by IT, add mpu6500_parse() before update.
void mpu6500_update(void)
{
	mpu6500_conv_data();
	mpu6500_complementary_filter(UPDATE_SEC);
}

// MPU6500 초기화
void mpu6500_init(void)
{
    uint8_t data;

    // ① 디바이스 리셋
    data = 0x80;
    mpu6500_write(0x6B, &data, 1);
    HAL_Delay(100);

    // ② 신호경로 리셋
    data = 0x07;
    mpu6500_write(0x68, &data, 1);
    HAL_Delay(100);

    // ③ 슬립 해제 + PLL 클럭
    data = 0x01;
    mpu6500_write(0x6B, &data, 1);
    HAL_Delay(35);

    // ④ 모든 축 활성화
    data = 0x00;
    mpu6500_write(0x6C, &data, 1);

    // ⑤ 자이로 DLPF (250Hz, 0.97ms)
    data = 0x00;
    mpu6500_write(0x1A, &data, 1);

    // ⑥ 자이로 ±2000dps, FCHOICE_B=00
    data = 0x18;
    mpu6500_write(0x1B, &data, 1);

    // ⑦ 가속도 ±16g
    data = 0x18;
    mpu6500_write(0x1C, &data, 1);

    // ⑧ 가속도 DLPF (460Hz, 1.94ms)
    data = 0x00;
    mpu6500_write(0x1D, &data, 1);

    // ⑨ 샘플레이트 1kHz (SMPLRT_DIV=0)
    data = 0x00;
    mpu6500_write(0x19, &data, 1);

    // ⑩ 인터럽트 핀: 액티브 하이, 푸시풀, 50us 펄스, 읽으면 클리어
    data = 0x10;
    mpu6500_write(0x37, &data, 1);

    // ⑪ 데이터 레디 인터럽트 활성화
    data = 0x01;
    mpu6500_write(0x38, &data, 1);

    // 안정화 대기
    HAL_Delay(100);
}



